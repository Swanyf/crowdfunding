		...
		<ul style="padding-left: 0px;" class="list-group">
			...
			<li class="list-group-item tree-closed"><span><i
				...
				<ul style="margin-top: 10px; display: none;">
					<li style="height: 30px;"><a href="user/page.html"><i
							class="glyphicon glyphicon-user"></i> 用户维护</a></li>
					<li style="height: 30px;"><a href="get/role/list.html"><i
							class="glyphicon glyphicon-king"></i> 角色维护</a></li>
					<li style="height: 30px;"><a href="permition/tree.html"><i
			...
			<%-- <c:if test="${empty sessionScope.root.children }">
				<li>获取菜单数据失败！请重新登录！</li>
			</c:if>
			<c:if test="${!empty sessionScope.root.children }">
				<c:forEach items="${sessionScope.root.children }" var="permission">
					<li class="list-group-item tree-closed"><i
						class="${permission.icon }"></i> ${permission.name } <c:if
							test="${!empty permission.children }">
							<ul style="margin-top: 10px; display: none;">
